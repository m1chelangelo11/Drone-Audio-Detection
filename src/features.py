...
def extract_features(
    processed_sample: list[tuple[np.ndarray, int, int]],
    sr: int,
    n_mfcc: int,
    n_fft: int,
    hop_length: int,
    ...
    """
    Extracts MFCC features from a preprocessed sample, then computes 
    delta and delta-delta factors
    ...
    try:
        ...
        for sample in processed_sample:
            mfcc_features = librosa.feature.mfcc(
                y=sample[0], sr=sr, n_mfcc=n_mfcc, n_fft=n_fft, hop_length=hop_length
            )

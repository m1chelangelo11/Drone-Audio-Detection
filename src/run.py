...
from preprocess import build_ambient_pool, AudioPreprocessing
...
def main(cfg: DictConfig) -> None:
    ...
    if (root / 'data' / 'features.npz').exists():
        ...
        for i in tqdm(range(len(ds['train']))):
            ...
            feature_vector = extract_features(preprocessed_sample,
                                            cfg.data.sr,
                                            cfg.data.n_mfcc,
                                            cfg.data.n_fft,
                                            cfg.data.hop_length)
    ...
    classifier, scaler, y_pred, X_test, y_test, X_val, y_val = train(X, y,
                                                                     groups,

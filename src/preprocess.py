...
def build_ambient_pool(ds: Any, seed: int, n: int = 500) -> List[np.ndarray]:
    ...
    mapped = no_drone_pool.map(lambda x: {"array": load_audio(x['audio'])[0]}, num_proc = 1)
...
class AudioPreprocessing:
    ...
    def __call__(self, sample: Any, label: int, group_id: int) -> List[Tuple[np.ndarray, int, int]]:
    ...
    def pad_audio(self, array: np.ndarray, 
                  sr: int, min_duration_s : Optional[float] = None) -> np.ndarray:
    ...
    def augment_pre_cut(self, array: np.ndarray, sr: int) -> np.ndarray:
        ...
        if np.random.random() < self.p:
            output = output + np.random.randn(len(output)).astype(np.float32) * amplitude

        if np.random.random() < self.p:
            ir = np.random.randn(int(sr * 0.5)) * np.exp(-np.linspace(0, 5, int(sr * 0.5)))
            output = scipy.signal.fftconvolve(output, ir, mode='same').astype(np.float32)

        if np.max(np.abs(output)) == 0:
            log.warning("Silent segment detected")
            return output
    ...
    def augment_post_cut(self, array: np.ndarray, sr: int) -> np.ndarray:
        """Applies post-cut audio augmentation (gain, shift, filter, stretch, pitch)
        ...
        if np.random.random() < self.p:
            sos = scipy.signal.butter(N=4, Wn=[self.low, self.high], btype='band',
                                    fs=sr, output='sos')
            output = scipy.signal.sosfilt(sos, output).astype(np.float32)
        
        if np.random.random() < self.p:
            output = librosa.effects.time_stretch(output,
                                                rate=np.random.uniform(0.8, 1.2))

        if np.random.random() < self.p:
            output = librosa.effects.pitch_shift(output, sr=sr,
